...
const TshirtCard: React.FC<TshirtCardProps> = ({ size, color, multiplicateur, rarity, description, price, totalMintable }) => {
  const imageSrc = `/shop_items/tshirt/${color.toLowerCase()}.png`;

  return (
    <div className="bg-gray-200 rounded-lg shadow-md p-4 flex items-center m-2">
      <img src={imageSrc} alt={`Tshirt ${color}`} className="w-1/4 h-16 mr-4" />
      <div className="w-1/2">
