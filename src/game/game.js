...
export const initializeGame = (initialMuted) => {
    ...
    function manageFlybyBird() {
        ...
        if (flyby_bird_x >= -50 && flyby_bird_x < screen_width() + 100) {
            ...
            if (!bird_sound_played && game_active && !isMuted) {
                flyby_sound.currentTime = 0; // Reset the sound to the beginning
                flyby_sound.play().catch(e => console.error("Error playing flyby sound:", e));
    ...
    function gameLoop() {
        ...
        if (game_active) {
            ...
            if (!end_sound_played && !isMuted) {
                end_sound.play().catch(e => console.error("Error playing end sound:", e));
    ...
    function resetGame() {
        ...
        if (!isMuted) {
            ...
            start_sound.play().catch(e => console.error("Error playing start sound:", e));
        }
        end_sound_played = false;
        if (high_score < score) {
            high_score = score;
    ...
    function handleUserInput() {
        if (!game_active) {
            ...
            bird_movement = -6;
            if (!isMuted) {
                flap_sound.currentTime = 0; // Reset the sound to the beginning
                flap_sound.play().catch(e => console.error("Error playing flap sound:", e));
            }
        }
    }
    ...
    gameControls = {
        ...
        setMuted: (muted) => {
            isMuted = muted;
            updateMuteState();
            if (!isMuted && game_active) {
                if (!musicPlaying) {
                    music.play().catch(e => console.error("Error playing music:", e));
                    musicPlaying = true;
                }
                // Play start sound when unmuting during an active game
                start_sound.play().catch(e => console.error("Error playing start sound:", e));
            }
        }
    };
